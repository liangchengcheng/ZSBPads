...
public class FavPresenterImpl implements FavPresenter {
    ...
    private FavModel model;
    private static final int DEF_DELAY = (int) (1 * 1000);

    public FavPresenterImpl(FavView view) {
        this.view = view;
        model = new FavModel();
    }
    ...
    private void loadData(final int page, String type, final boolean get_data) {
        ...
        model.getFavList(page, type, new ResultCallback<String>() {
            @Override
            ...
            public void run() {
                try {
                    ...
                    if (status == 1) {
                        ...
                        List<FavEntity> weekDatas = GsonUtils.fromJsonArray(result, FavEntity.class);
                        if (page == 1) {
